...
from algo.randwalk import randwalk, randwalk_2


def analyze_root_dycause(
    transition_matrix,
    entry_point,
    local_data,
    epoch=1000,
...
def analyze_root_2(
    ...
    out_path, new_matrix = randwalk_2(
        transition_matrix,
        peak,
        epoch=epoch,
        mean_method=mean_method,
...
import numpy as np
a = np.zeros((600, 1)).reshape(1, -1)
b= np.zeros((600, 1)).reshape(1, -1)
c = np.concatenate([a, b], axis=0)
c.shape

# ?np.corrcoef

print("{:^0}{:<5.4f},{}".format("", 0.12341235, [1,4,2,3]))

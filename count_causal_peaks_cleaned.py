...
# +
resPathListPure = [[] for item in lengthList]
resPathList = [[] for item in lengthList]
resPathListUni = [[] for item in lengthList]
peakNumList = [0 for item in lengthList]
peakNumListUni = [0 for item in lengthList]
...
for i, befaftlength in enumerate(lengthList):
    ...
    config["Granger params"]["auto_threshold_ratio"] = 0.2
...
#                     break
#             if flag: continue
#             q.put(path + [[x_i, local_length]])
#         if q.qsize() == lasts:
#             resPathsPure.append(path)

#     resPathListPure[i] = resPathsPure
...
#             if peakWeiList[x_i][las][0] == length - 1: continue
    ...
    for x_i in range(varNum):
        for y_i in range(varNum):
            ...
            peakNum += len(peakPosList[x_i][y_i])
    peakNumList[i] = peakNum
    ...
    for x_i in range(varNum):
        for y_i in range(varNum):
            ...
            if matrix[x_i][y_i] == 0: continue
            peakNum += 1
    peakNumListUni[i] = peakNum
    
    print(f"{lengthList[i]:3d} {len(resPathListPure[i]):5d} {len(resPathList[i]):5d} {len(resPathListUni[i]):5d} {peakNumList[i]:3d} {peakNumListUni[i]:3d}")
# -

for i in range(len(lengthList)):
    print(f"{lengthList[i]:3d} {len(resPathList[i]):5d} {len(resPathListUni[i]):5d}")
